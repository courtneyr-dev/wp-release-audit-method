...
> [!IMPORTANT]
> **Amended 2026-08-13 — one rule added, not a rewrite.** As first run, this prompt's
> `ADJUDICATE BEFORE YOU BUILD` rule did not exist: the "none may become a test before source
> adjudication" warning lived only in the lens sections *below* the pasted block, so an agent
> handed the block never saw it. The rule now sits in `<method_rules>` and in the completeness
> contract, where the agent reads it. Everything else is byte-identical to the version whose
> results the audit records cite — so a handoff citing this prompt should note the amendment
> rather than claim this exact text produced those earlier results.
...
- ADJUDICATE BEFORE YOU BUILD. A chain proposal carries a register status: REVISE (source-
  adjudicated, buildable), DROP (refuted, kept as a negative control), or UNADJUDICATED. Build
  ONLY from REVISE. An UNADJUDICATED proposal — however plausible, and whatever lens produced it
  — becomes a test only after you have read the relevant WordPress source and adjudicated it,
  which may itself turn it into a DROP. The reason is measured, not theoretical: of the first 36
  proposals here, generated by four independent lenses, **zero survived adjudication unchanged**
  — 16 were refuted on verified-false premises and 20 needed correction. A suite built from
  unadjudicated proposals tests things that cannot happen, and reports the resulting silence as
  evidence.
</method_rules>
...
Done means: every chain built traces to a register row whose status is REVISE, with the source
you adjudicated it against cited; every chain has a register row; every matrix cell has a verdict or an exact blocker;
