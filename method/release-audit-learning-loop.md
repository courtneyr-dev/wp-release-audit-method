...
- **Activation is not a boot — but boot it over HTTP, not WP-CLI** (added 2026-08-23, **receipt refuted and rule corrected 2026-08-24**). A plugin can install and activate cleanly and still fatal on the next load. That much stands. What does *not* stand is the receipt this rule shipped with, or the instrumentation it recommended.

  The original receipt was `eps-301-redirects` 2.85 fatalling when WP-CLI loaded WordPress with it active. **That fatal is a WP-CLI artifact, not a plugin bug.** WP-CLI requires `wp-settings.php` from inside `WP_CLI\Runner->load_wordpress()`, so a plugin's file-scope `$Var = new Thing()` becomes a local of that method rather than a global; the plugin then reads it back with `global $Var` on `init` and gets null. Over HTTP the same file is included at global scope and everything works — the site serves 200 with no `debug.log`. Verified here independently, in plain PHP with no WordPress involved: a file included at global scope creates globals, the same file included inside a method does not.

  So the corrected rule: **no plugin cell is clean until an HTTP request has been served with the plugin active.** A WP-CLI boot is still worth running, but a CLI-only fatal is a **note, not a failure** — it is as likely to be this scoping artifact as a real defect. Run it last, after the HTTP checks, so its fatal doesn't also poison a `debug.log` assertion.

  This is a sharper form of the two-boots rule in [preflight-core-probe](preflight-core-probe.md), and it corrects that page too: CLI is not a *smaller* denominator than HTTP, it is a **different** one that manufactures failures HTTP never produces. See [ecosystem-compatibility-lane](ecosystem-compatibility-lane.md#activation-is-not-a-boot).
