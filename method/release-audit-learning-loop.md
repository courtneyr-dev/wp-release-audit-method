...
- **Activation is not a boot** (added 2026-08-23). A plugin can install and activate cleanly and still fatal on the next load — activation runs a narrow path, often before the hooks, globals, and co-installed plugins the plugin's real code assumes. The receipt is external and executed: in the first full 200-plugin run of the workflow proposed in [Trac #65920](https://core.trac.wordpress.org/ticket/65920), the single real failure was `eps-301-redirects` 2.85, which activated fine and then fatalled on `null` when WP-CLI loaded WordPress with it active. Checkable step: no plugin cell is clean until WordPress has been **loaded** with the plugin active and the load produced no fatal — `wp eval 'echo 1;'` after `--activate` is the whole check. Same family as the two-boots rule in [preflight-core-probe](preflight-core-probe.md); see [ecosystem-compatibility-lane](ecosystem-compatibility-lane.md#activation-is-not-a-boot).
